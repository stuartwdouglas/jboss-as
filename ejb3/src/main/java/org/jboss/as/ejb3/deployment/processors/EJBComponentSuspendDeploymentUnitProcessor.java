        ...
        for (ComponentDescription component : deploymentUnit.getAttachment(Attachments.EE_MODULE_DESCRIPTION).getComponentDescriptions()) {
            if (component instanceof EJBComponentDescription) {
                ...
                component.getConfigurators().add((context1, description, configuration) -> {
                    ...
                    configuration.getCreateDependencies().add((DependencyConfigurator<EJBComponentCreateService>) (serviceBuilder, service) -> serviceBuilder.addDependency(ControlPointService.serviceName(topLevelName, entryPoint), ControlPoint.class, service.getControlPointInjector()));
