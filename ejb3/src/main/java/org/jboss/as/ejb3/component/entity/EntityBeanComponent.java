...
public class EntityBeanComponent extends EJBComponent {
    ...
    protected EntityBeanComponent(final EntityBeanComponentCreateService ejbComponentCreateService) {
        ...
        if (poolConfig == null) {
            ROOT_LOGGER.debug("Pooling is disabled for Entity EJB " + ejbComponentCreateService.getComponentName());
            this.pool = null;
        } else {
            ROOT_LOGGER.debug("Using pool config " + poolConfig + " to create pool for Entity EJB " + ejbComponentCreateService.getComponentName());
