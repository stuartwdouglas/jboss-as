...
public class RaRemove implements OperationStepHandler {
    ...
    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
        ...
        context.addStep((context13, operation13) -> {
            ...
            wasActive = RaOperationUtil.removeIfActive(context13, archiveOrModuleName, idName);

            if (wasActive) {
                if(!context13.isResourceServiceRestartAllowed()) {
                    context13.reloadRequired();
                    context13.completeStep((context12, operation12) -> context12.revertReloadRequired());
                    return;
                }
            }

            ServiceName raServiceName = ServiceName.of(ConnectorServices.RA_SERVICE, idName);
            ServiceController<?> serviceController =  context13.getServiceRegistry(false).getService(raServiceName);
            ...
            final List<ServiceName> serviceNameList = context13.getServiceRegistry(false).getServiceNames();
            for (ServiceName name : serviceNameList) {
                if (raServiceName.isParentOf(name)) {
                    context13.removeService(name);
                }
            ...
            if (model.get(MODULE.getName()).isDefined()) {
                ...
                context13.removeService(deployerServiceName);
                ServiceName inactiveServiceName = ConnectorServices.INACTIVE_RESOURCE_ADAPTER_SERVICE.append(idName);
                context13.removeService(inactiveServiceName);
            }


            context13.removeService(raServiceName);
            context13.completeStep((context1, operation1) -> {
                if (resourceAdapter != null) {
                    List<ServiceController<?>>  newControllers = new LinkedList<ServiceController<?>>();
                    if (model.get(ARCHIVE.getName()).isDefined()) {
                        RaOperationUtil.installRaServices(context1, idName, resourceAdapter, newControllers);
                    } else {
                        try {
                            RaOperationUtil.installRaServicesAndDeployFromModule(context1, idName, resourceAdapter, archiveOrModuleName, newControllers);
                    ...
                    try {
                        if (wasActive){
                            RaOperationUtil.activate(context1, idName, archiveOrModuleName);
