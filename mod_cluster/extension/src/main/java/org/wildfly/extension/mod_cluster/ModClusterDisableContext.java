    ...
    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
        if (context.isNormalServer() && context.getServiceRegistry(false).getService(ContainerEventHandlerService.SERVICE_NAME) != null) {
            context.addStep((context12, operation12) -> {
                ...
                ROOT_LOGGER.debugf("disable-context: %s", operation12);

                final String webHost = VIRTUAL_HOST.resolveModelAttribute(context12, operation12).asString();
                final String webContext = CONTEXT.resolveModelAttribute(context12, operation12).asString();
                ...
                context12.completeStep((context1, operation1) -> service.enableContext(webHost, webContext));
