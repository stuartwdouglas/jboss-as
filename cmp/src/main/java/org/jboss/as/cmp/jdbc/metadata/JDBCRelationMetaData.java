...
public final class JDBCRelationMetaData {
    ...
    public static JDBCRelationMetaData merge(final JDBCApplicationMetaData jdbcApplication, final JDBCRelationMetaData defaultValues, final JDBCRelationMetaData newValues) {
        ...
        relationMetaData.tableName = tableNameString;
        ...
        JDBCRelationshipRoleMetaData defaultLeft = defaultValues.getLeftRelationshipRole();
        JDBCRelationshipRoleMetaData defaultRight = defaultValues.getRightRelationshipRole();
        if (newValues.roles.isEmpty()) {
            relationMetaData.left = new JDBCRelationshipRoleMetaData(relationMetaData, jdbcApplication, defaultLeft);
            relationMetaData.right = new JDBCRelationshipRoleMetaData(relationMetaData, jdbcApplication, defaultRight);
            relationMetaData.left.init(relationMetaData.right);
            relationMetaData.right.init(relationMetaData.left);
        } else {
            JDBCRelationshipRoleMetaData leftParsed = getEJBRelationshipRole(newValues.roles, defaultLeft);
            relationMetaData.left = new JDBCRelationshipRoleMetaData(relationMetaData, jdbcApplication, leftParsed, defaultLeft);

            JDBCRelationshipRoleMetaData rightParsed = getEJBRelationshipRole(newValues.roles, defaultRight);
            relationMetaData.right = new JDBCRelationshipRoleMetaData(relationMetaData, jdbcApplication, rightParsed, defaultRight);

            relationMetaData.left.init(relationMetaData.right);
            relationMetaData.right.init(relationMetaData.left);
        }

        relationMetaData.relationName = newValues.relationName != null ? newValues.relationName : getNonNullRelationName(relationMetaData.left, relationMetaData.right, defaultValues.getRelationName());

        // at least one side of a fk relation must have keys
        if (relationMetaData.isForeignKeyMappingStyle() && relationMetaData.left.getKeyFields().isEmpty() && relationMetaData.right.getKeyFields().isEmpty()) {
            throw new RuntimeException("Atleast one role of a foreign-key "
                    + "mapped relationship must have key fields " + "(or <primkey-field> is missing from ejb-jar.xml): "
                    + "ejb-relation-name=" + relationMetaData.relationName);
        }

        // both sides of a table relation must have keys
        if (relationMetaData.isTableMappingStyle() && (relationMetaData.left.getKeyFields().isEmpty() || relationMetaData.right.getKeyFields().isEmpty())) {
            throw new RuntimeException("Both roles of a relation-table " + "mapped relationship must have key fields: "
                    + "ejb-relation-name=" + relationMetaData.relationName);
        }
    ...
    private static JDBCRelationshipRoleMetaData getEJBRelationshipRole(List<JDBCRelationshipRoleMetaData> roles, JDBCRelationshipRoleMetaData defaultRole) {
        ...
        for (JDBCRelationshipRoleMetaData role : roles) {
     ...
     * For parsing only.
     *
     * @param jdbcRelationshipRoleMetaData
     */
    public void addEjbRelationshipRole(final JDBCRelationshipRoleMetaData jdbcRelationshipRoleMetaData) {
        roles.add(jdbcRelationshipRoleMetaData);
    }
