...
public class JDBCQueryMetaDataFactory {
    ...
    public List<JDBCQueryMetaData> createJDBCQueryMetaData(final Type type, final String methodName, final List<String> params, final String query, final JDBCReadAheadMetaData readAheadMetaData, final Class<?> qlCompiler, final boolean lazyResultSet, final Map<String, String> props) {
        final Class<?>[] parameters;
        try {
            parameters = Classes.convertToJavaClasses(params.iterator(), entity.getJDBCApplication().getClassLoader());
        } catch (ClassNotFoundException e) {
            throw new RuntimeException("Failed to convert method params to class instances: " + params);
        }
        final List<Method> methods = getQueryMethods(methodName, parameters);
        ...
        for (Method method : methods) {
            ...
            switch (type) {
                case RAW_SQL: {
                    queryMetaData = new JDBCRawSqlQueryMetaData(method, qlCompiler, lazyResultSet);
                    break;
                }
                case JBOSS_QL: {
                    queryMetaData = new JDBCJBossQLQueryMetaData(isResultTypeMappingLocal, query, method, readAheadMetaData, qlCompiler, lazyResultSet);
                    break;
                }
                case DYNAMIC_QL: {
                    queryMetaData = new JDBCDynamicQLQueryMetaData(isResultTypeMappingLocal, method, readAheadMetaData, qlCompiler, lazyResultSet);
                    break;
                }
                case DECLARED_QL: {
                    queryMetaData = new JDBCDeclaredQueryMetaData(isResultTypeMappingLocal, method, readAheadMetaData, qlCompiler, lazyResultSet, props);
                    break;
                }
                default: {
                    if(defaultValue != null && defaultValue instanceof JDBCQlQueryMetaData) {
                        queryMetaData = new JDBCQlQueryMetaData((JDBCQlQueryMetaData) defaultValue, readAheadMetaData, qlCompiler, false);
                    } else {
