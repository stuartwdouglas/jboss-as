 ...
 */
public class SunORBSubsystemParser implements XMLStreamConstants, XMLElementReader<List<ModelNode>>,
        XMLElementWriter<SubsystemMarshallingContext> {

    static final SunORBSubsystemParser INSTANCE = new SunORBSubsystemParser();
    ...
    public void readElement(XMLExtendedStreamReader reader, List<ModelNode> nodes) throws XMLStreamException {
        ...
        subsystem.get(OP_ADDR).add(SUBSYSTEM, SunORBExtension.SUBSYSTEM_NAME);
        ...
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            ...
            switch (element) {
                case PROPERTY: {
                    ...
                    this.parseGenericProperty(reader, node.get(SunORBSubsystemConstants.PROPERTIES));
                    break;
                }
                default: {
                    throw unexpectedElement(reader);
    ...
    private void parseAttributes(XMLExtendedStreamReader reader, ModelNode node, EnumSet<Attribute> expectedAttributes,
        ...
        for (int i = 0; i < reader.getAttributeCount(); i++) {
            ...
            ((SimpleAttributeDefinition) SunORBSubsystemDefinitions.valueOf(attribute.getLocalName())).
    ...
    public void writeContent(XMLExtendedStreamWriter writer, SubsystemMarshallingContext context) throws XMLStreamException {
        ...
        String properties = SunORBSubsystemConstants.PROPERTIES;
     ...
     * @throws javax.xml.stream.XMLStreamException
     *          if an error occurs while writing the ORB configuration.
     */
    private void writeORBConfig(XMLExtendedStreamWriter writer, ModelNode node) throws XMLStreamException {

        boolean writeORB = this.isWritable(node, SunORBSubsystemDefinitions.ORB_ATTRIBUTES);
        boolean writeORBConnection = this.isWritable(node, SunORBSubsystemDefinitions.ORB_CONN_ATTRIBUTES);
        boolean writeORBInitializer = this.isWritable(node, SunORBSubsystemDefinitions.ORB_INIT_ATTRIBUTES);
        ...
        if (!writeORBConnection && !writeORBInitializer) {
            if (writeORB) {
                writer.writeEmptyElement(SunORBSubsystemConstants.ORB);
                this.writeAttributes(writer, node, SunORBSubsystemDefinitions.ORB_ATTRIBUTES);
            }
        }
        // otherwise write the orb element with the appropriate sub-elements.
        else {
            writer.writeStartElement(SunORBSubsystemConstants.ORB);
            this.writeAttributes(writer, node, SunORBSubsystemDefinitions.ORB_ATTRIBUTES);
            if (writeORBConnection) {
                writer.writeEmptyElement(SunORBSubsystemConstants.ORB_CONN);
                this.writeAttributes(writer, node, SunORBSubsystemDefinitions.ORB_CONN_ATTRIBUTES);
            }
            if (writeORBInitializer) {
                writer.writeEmptyElement(SunORBSubsystemConstants.ORB_INIT);
                this.writeAttributes(writer, node, SunORBSubsystemDefinitions.ORB_INIT_ATTRIBUTES);
            }
     ...
     * @throws javax.xml.stream.XMLStreamException
     *          if an error occurs while writing the POA configuration.
     */
    private void writePOAConfig(XMLExtendedStreamWriter writer, ModelNode node) throws XMLStreamException {

        boolean writePOA = this.isWritable(node, SunORBSubsystemDefinitions.POA_ATTRIBUTES);
        boolean writePOARP = this.isWritable(node, SunORBSubsystemDefinitions.POA_RP_ATTRIBUTES);

        // if no request processor properties are available, just write the poa properties (if any) in an empty element.
        if (!writePOARP) {
            if (writePOA) {
                writer.writeEmptyElement(SunORBSubsystemConstants.POA);
                this.writeAttributes(writer, node, SunORBSubsystemDefinitions.POA_ATTRIBUTES);
            }
        }
        // otherwise write the poa element with the appropriate sub-elements.
        else {
            writer.writeStartElement(SunORBSubsystemConstants.POA);
            this.writeAttributes(writer, node, SunORBSubsystemDefinitions.POA_ATTRIBUTES);
            writer.writeEmptyElement(SunORBSubsystemConstants.POA_RP);
            this.writeAttributes(writer, node, SunORBSubsystemDefinitions.POA_RP_ATTRIBUTES);
            writer.writeEndElement();
        }
    }
     ...
     * @throws javax.xml.stream.XMLStreamException
     *          if an error occurs while writing the interop configuration.
     */
    private void writeNamingConfig(XMLExtendedStreamWriter writer, ModelNode node) throws XMLStreamException {
        boolean writeNaming = this.isWritable(node, SunORBSubsystemDefinitions.NAMING_ATTRIBUTES);
        if (writeNaming) {
            writer.writeEmptyElement(SunORBSubsystemConstants.NAMING);
            this.writeAttributes(writer, node, SunORBSubsystemDefinitions.NAMING_ATTRIBUTES);
        }
    }
    ...
    private void writeInteropConfig(XMLExtendedStreamWriter writer, ModelNode node) throws XMLStreamException {
        boolean writeInterop = this.isWritable(node, SunORBSubsystemDefinitions.INTEROP_ATTRIBUTES);
        if (writeInterop) {
            writer.writeEmptyElement(SunORBSubsystemConstants.INTEROP);
            this.writeAttributes(writer, node, SunORBSubsystemDefinitions.INTEROP_ATTRIBUTES);
        }
    }
     ...
     * @throws javax.xml.stream.XMLStreamException
     *          if an error occurs while writing the security configuration.
     */
    private void writeSecurityConfig(XMLExtendedStreamWriter writer, ModelNode node) throws XMLStreamException {
        boolean writeSecurity = this.isWritable(node, SunORBSubsystemDefinitions.SECURITY_ATTRIBUTES);
        if (writeSecurity) {
            writer.writeEmptyElement(SunORBSubsystemConstants.SECURITY);
            this.writeAttributes(writer, node, SunORBSubsystemDefinitions.SECURITY_ATTRIBUTES);
        }
    }
    ...
    private void writeGenericProperties(XMLExtendedStreamWriter writer, ModelNode node) throws XMLStreamException {
        writer.writeStartElement(SunORBSubsystemConstants.PROPERTIES);
        for (Property prop : node.asPropertyList()) {
            writer.writeEmptyElement(SunORBSubsystemConstants.PROPERTY);
            writer.writeAttribute(SunORBSubsystemConstants.NAME, prop.getName());
            writer.writeAttribute(SunORBSubsystemConstants.PROPERTY_VALUE, prop.getValue().asString());
        }
        writer.writeEndElement();
    }
     ...
     * @throws javax.xml.stream.XMLStreamException
     *          if an error occurs while writing the attributes to the current element.
     */
    ...
    enum Namespace {
        ...
        UNKNOWN(null),
        SUN_ORB_1_0("urn:jboss:domain:sunorb:1.0");

        static final Namespace CURRENT = SUN_ORB_1_0;
    ...
    enum Element {
        ...
        ORB(SunORBSubsystemConstants.ORB),
        ORB_CONNECTION(SunORBSubsystemConstants.ORB_CONN),
        ORB_INITIALIZERS(SunORBSubsystemConstants.ORB_INIT),

        // elements used to configure the POA.
        POA(SunORBSubsystemConstants.POA),
        POA_REQUEST_PROC(SunORBSubsystemConstants.POA_RP),

        // elements used to configure the naming service, ORB interoperability and ORB security.
        NAMING(SunORBSubsystemConstants.NAMING),
        INTEROP(SunORBSubsystemConstants.INTEROP),
        SECURITY(SunORBSubsystemConstants.SECURITY),

        // elements used to configure generic properties.
        PROPERTIES(SunORBSubsystemConstants.PROPERTIES),
        PROPERTY(SunORBSubsystemConstants.PROPERTY);
    ...
    enum Attribute {
        ...
        NAME(SunORBSubsystemConstants.NAME),
        ORB_PRINT_VERSION(SunORBSubsystemConstants.ORB_PRINT_VERSION),
        ORB_USE_IMR(SunORBSubsystemConstants.ORB_USE_IMR),
        ORB_USE_BOM(SunORBSubsystemConstants.ORB_USE_BOM),
        ORB_CACHE_TYPECODES(SunORBSubsystemConstants.ORB_CACHE_TYPECODES),
        ORB_CACHE_POA_NAMES(SunORBSubsystemConstants.ORB_CACHE_POA_NAMES),
        ORB_GIOP_MINOR_VERSION(SunORBSubsystemConstants.ORB_GIOP_MINOR_VERSION),
        ORB_SOCKET_BINDING(SunORBSubsystemConstants.ORB_SOCKET_BINDING),
        ORB_SSL_SOCKET_BINDING(SunORBSubsystemConstants.ORB_SSL_SOCKET_BINDING),

        // attributes of the connection element.
        ORB_CONN_RETRIES(SunORBSubsystemConstants.ORB_CONN_RETRIES),
        ORB_CONN_RETRY_INTERVAL(SunORBSubsystemConstants.ORB_CONN_RETRY_INTERVAL),
        ORB_CONN_CLIENT_TIMEOUT(SunORBSubsystemConstants.ORB_CONN_CLIENT_TIMEOUT),
        ORB_CONN_SERVER_TIMEOUT(SunORBSubsystemConstants.ORB_CONN_SERVER_TIMEOUT),
        ORB_CONN_MAX_SERVER_CONNECTIONS(SunORBSubsystemConstants.ORB_CONN_MAX_SERVER_CONNECTIONS),
        ORB_CONN_MAX_MANAGED_BUF_SIZE(SunORBSubsystemConstants.ORB_CONN_MAX_MANAGED_BUF_SIZE),
        ORB_CONN_OUTBUF_SIZE(SunORBSubsystemConstants.ORB_CONN_OUTBUF_SIZE),
        ORB_CONN_OUTBUF_CACHE_TIMEOUT(SunORBSubsystemConstants.ORB_CONN_OUTBUF_CACHE_TIMEOUT),

        // attributes of the initializers element.
        ORB_INIT_SECURITY(SunORBSubsystemConstants.ORB_INIT_SECURITY),
        ORB_INIT_TRANSACTIONS(SunORBSubsystemConstants.ORB_INIT_TRANSACTIONS),

        // attributes of the poa element.
        POA_MONITORING(SunORBSubsystemConstants.POA_MONITORING),
        POA_QUEUE_WAIT(SunORBSubsystemConstants.POA_QUEUE_WAIT),
        POA_QUEUE_MIN(SunORBSubsystemConstants.POA_QUEUE_MIN),
        POA_QUEUE_MAX(SunORBSubsystemConstants.POA_QUEUE_MAX),

        // attributes of the request-processor element.
        POA_REQUEST_PROC_POOL_SIZE(SunORBSubsystemConstants.POA_RP_POOL_SIZE),
        POA_REQUEST_PROC_MAX_THREADS(SunORBSubsystemConstants.POA_RP_MAX_THREADS),

        // attributes of the naming element - the ORB service will build the relevant JacORB properties from these values.
        NAMING_EXPORT_CORBALOC(SunORBSubsystemConstants.NAMING_EXPORT_CORBALOC),
        NAMING_ROOT_CONTEXT(SunORBSubsystemConstants.NAMING_ROOT_CONTEXT),

        // attributes of the interop element.
        INTEROP_SUN(SunORBSubsystemConstants.INTEROP_SUN),
        INTEROP_COMET(SunORBSubsystemConstants.INTEROP_COMET),
        INTEROP_IONA(SunORBSubsystemConstants.INTEROP_IONA),
        INTEROP_CHUNK_RMI_VALUETYPES(SunORBSubsystemConstants.INTEROP_CHUNK_RMI_VALUETYPES),
        INTEROP_LAX_BOOLEAN_ENCODING(SunORBSubsystemConstants.INTEROP_LAX_BOOLEAN_ENCODING),
        INTEROP_INDIRECTION_ENCODING_DISABLE(SunORBSubsystemConstants.INTEROP_INDIRECTION_ENCODING_DISABLE),
        INTEROP_STRICT_CHECK_ON_TC_CREATION(SunORBSubsystemConstants.INTEROP_STRICT_CHECK_ON_TC_CREATION),

        // attributes of the security element.
        SECURITY_SUPPORT_SSL(SunORBSubsystemConstants.SECURITY_SUPPORT_SSL),
        SECURITY_SECURITY_DOMAIN(SunORBSubsystemConstants.SECURITY_SECURITY_DOMAIN),
        SECURITY_ADD_COMPONENT_INTERCEPTOR(SunORBSubsystemConstants.SECURITY_ADD_COMP_VIA_INTERCEPTOR),
        SECURITY_CLIENT_SUPPORTS(SunORBSubsystemConstants.SECURITY_CLIENT_SUPPORTS),
        SECURITY_CLIENT_REQUIRES(SunORBSubsystemConstants.SECURITY_CLIENT_REQUIRES),
        SECURITY_SERVER_SUPPORTS(SunORBSubsystemConstants.SECURITY_SERVER_SUPPORTS),
        SECURITY_SERVER_REQUIRES(SunORBSubsystemConstants.SECURITY_SERVER_REQUIRES),
        // if enabled the ORB service will configure JacORB to use the JBoss SSL socket factory classes by building the
        // appropriate properties.
        SECURITY_USE_DOMAIN_SF(SunORBSubsystemConstants.SECURITY_USE_DOMAIN_SF),
        SECURITY_USE_DOMAIN_SSF(SunORBSubsystemConstants.SECURITY_USE_DOMAIN_SSF),

        // attributes of the generic property element.
        PROP_KEY(SunORBSubsystemConstants.PROPERTY_KEY),
        PROP_VALUE(SunORBSubsystemConstants.PROPERTY_VALUE);
