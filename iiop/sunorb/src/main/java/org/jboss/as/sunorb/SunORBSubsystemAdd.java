...
import org.jboss.as.iiop.deployment.IIOPMarkerProcessor;
import org.jboss.as.iiop.naming.jndi.JBossCNCtxFactory;
 ...
 */
public class SunORBSubsystemAdd extends AbstractAddStepHandler {

    static final SunORBSubsystemAdd INSTANCE = new SunORBSubsystemAdd();
     ...
     * </p>
     */
    private SunORBSubsystemAdd() {
    }

    @Override
    protected void populateModel(ModelNode operation, ModelNode model) throws OperationFailedException {
        // populate the submodel.
        for (AttributeDefinition attrDefinition : SunORBSubsystemDefinitions.SUBSYSTEM_ATTRIBUTES) {
    ...
    protected void performRuntime(OperationContext context, ModelNode operation, ModelNode model,
        ...
        context.addStep(new AbstractDeploymentChainStep() {
            public void execute(DeploymentProcessorTarget processorTarget) {
                processorTarget.addDeploymentProcessor(SunORBExtension.SUBSYSTEM_NAME, Phase.DEPENDENCIES, Phase.DEPENDENCIES_JACORB, new SunORBDependencyProcessor());
                processorTarget.addDeploymentProcessor(SunORBExtension.SUBSYSTEM_NAME, Phase.PARSE, Phase.PARSE_JACORB, new IIOPMarkerProcessor());
            }
        ...
        SunCorbaORBService orbService = new SunCorbaORBService(props);
        final ServiceBuilder<ORB> builder = context.getServiceTarget().addService(
                IIOPServiceNames.ORB_SERVICE_NAME, orbService);
        ...
        String securityDomain = props.getProperty(SunORBSubsystemConstants.SECURITY_SECURITY_DOMAIN);
        ...
        String socketBinding = props.getProperty(SunORBSubsystemConstants.ORB_SOCKET_BINDING);
        builder.addDependency(SocketBinding.JBOSS_BINDING_NAME.append(socketBinding), SocketBinding.class,
                orbService.getSunORBSocketBindingInjector());
        String sslSocketBinding = props.getProperty(SunORBSubsystemConstants.ORB_SSL_SOCKET_BINDING);
        builder.addDependency(SocketBinding.JBOSS_BINDING_NAME.append(sslSocketBinding), SocketBinding.class,
                orbService.getSunORBSSLSocketBindingInjector());
        ...
        newControllers.add(context.getServiceTarget().addService(IIOPServiceNames.ROOT_SERVICE_NAME, rootPOAService).
                addDependency(IIOPServiceNames.ORB_SERVICE_NAME, ORB.class, rootPOAService.getORBInjector()).
        ...
        newControllers.add(context.getServiceTarget().addService(IIOPServiceNames.NAMING_SERVICE_NAME, namingService).
                ...
                addDependency(IIOPServiceNames.POA_SERVICE_NAME.append("namingpoa"), POA.class,
    ...
    private Properties getConfigurationProperties(OperationContext context, ModelNode model) throws OperationFailedException {
        ...
        for (AttributeDefinition attrDefinition : SunORBSubsystemDefinitions.SUBSYSTEM_ATTRIBUTES) {
            ...
            if (resolvedModelAttribute.isDefined()) {
                ...
                String jacorbProperty = PropertiesMap.SUN_ORB_PROPS_MAP.get(name);
                ...
                if (SunORBSubsystemDefinitions.SSL_CONFIG_ATTRIBUTES.contains(attrDefinition)) {
        ...
        if (model.hasDefined(SunORBSubsystemConstants.PROPERTIES)) {
            ModelNode propertiesNode = model.get(SunORBSubsystemConstants.PROPERTIES);
    ...
    private void setupInitializers(Properties props) {
        ...
        String installSecurity = (String) props.remove(SunORBSubsystemConstants.ORB_INIT_SECURITY);
        ...
        for (String initializerClass : orbInitializers)
            props.setProperty(SunORBSubsystemConstants.ORB_INITIALIZER_PREFIX + initializerClass, "");
    }
    ...
    private void setupSSLFactories(final Properties props) throws OperationFailedException {
        ...
        if (supportSSL) {
            ...
            String securityDomain = props.getProperty(SunORBSubsystemConstants.SECURITY_SECURITY_DOMAIN);
            if (securityDomain == null || securityDomain.isEmpty())
                throw SunORBMessages.MESSAGES.noSecurityDomainSpecified();
